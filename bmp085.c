...
void bmp085_start_rawtemperature(void){
	bmp085_writemem(BMP085_REGCONTROL, BMP085_REGREADTEMPERATURE);
	bmp180_temp_conv_state = BMPCS_TCONV;
}

void bmp085_read_rawtemperature(void){
	uint8_t buff[2];
	memset(buff, 0, sizeof(buff));
	long ut,x1,x2;
...
void bmp085_start_rawpressure(void){
#if BMP085_AUTOUPDATETEMP == 1
	bmp085_getrawtemperature();
#endif

	bmp085_writemem(BMP085_REGCONTROL, BMP085_REGREADPRESSURE+(BMP085_MODE << 6));
	bmp180_press_conv_state = BMPCS_PCONV;
}
...
void bmp085_read_rawpressure(void){
	...
	bmp085_rawpressure = bmp085_avaragefilter(bmp085_rawpressure);
	#endif
	bmp180_press_conv_state = BMPCS_PUNINIT;
}
...
double bmp085_gettemperature() {
	//bmp085_getrawtemperature();
	switch(bmp180_temp_conv_state){
		case BMPCS_TUNINIT:
		{
			bmp085_start_rawtemperature();
			break;
		}
		case BMPCS_TCONV:
		{
			bmp085_read_rawtemperature();
			break;
		}
		default:
			break;
